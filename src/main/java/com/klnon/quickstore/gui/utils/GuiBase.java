...
public abstract class GuiBase extends Screen {
    public static final ResourceLocation BG_NORMAL = new ResourceLocation(QuickStore.PREFIX_GUI + "bg.png");
    public static final ResourceLocation BG_LARGE = new ResourceLocation(QuickStore.PREFIX_GUI + "bg-help.png");

    private boolean hasSide;
    private String sideTitle = "";
    private int backgroundWidth = BASE_WIDTH;
    private int backgroundHeight = BASE_HEIGHT;
    ...
    public void render(MatrixStack stack, int x, int y, float partialTicks) {
        ...
        if( this.hasSide ) {
            ...
            drawTexturedQuadFit((float) width / 2 + BASE_X_OFFSET, (float) height / 2 + BASE_Y_OFFSET, this.backgroundWidth, this.backgroundHeight, 0xffffff);

            if( hasSideTitle() )
                getFontRender().drawStringWithShadow(stack, this.sideTitle, (float) width / 2 + SIDE_TITLE_X_OFFSET, (float) height / 2 + SIDE_TITLE_Y_OFFSET, 0xffff00);
        }
